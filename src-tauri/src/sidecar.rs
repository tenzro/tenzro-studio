...
/// Layers our hardware-adaptive defaults (GPU layers, batch sizes,
/// ctx, jinja) on top of each discovered model, plus a
/// `chat_template_file` override for any model whose stem matches a
/// [`TEMPLATE_OVERRIDES`] prefix. Values come from the active
/// [`HardwareProfile`] so the preset file and the top-level CLI
/// flags stay in sync.
