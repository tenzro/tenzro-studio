...
/// llama-server process whose parent is `tenzro-studio` (so `ppid!=1` →
...
pub async fn restart_sidecar(
    ...
    let fresh = spawn_sidecar(app).await?;
    {
        ...
        if let Some(stray) = guard.take() {
            ...
            // non-orphan stray (parent is still tenzro-studio).
...
async fn generate_models_preset(
    ...
    ini.push_str("# Generated by tenzro-studio on app start.\n");
